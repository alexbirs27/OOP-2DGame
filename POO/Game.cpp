...
std::istream& operator>>(std::istream& input, GameObject* Player)
{
    int x, y;
    input >> x;
    input >> y;
...
void Game::init(const char* title, int xpos, int ypos, int width, int height, bool fullscreen)
{
   ...
   // GlfB = new pereche();
   // input >> GlfB;
    std::ifstream input("date.in.txt");
    GolfBall = new GameObject("minge_golf.png");
    input >> GolfBall;
    Golf1 = new GameObject("minge_golf.png");
    input >> Golf1;
    map = new Map();
